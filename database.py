...
def query_reddit():
  ...
  document_content_description = "The opinion of a user."
  metadata_field_info=[
    AttributeInfo(
        name="author",
        description="A unique username for the author of the comment", 
        type="string or list[string]", 
    ),
    AttributeInfo(
        name="comment_length",
        description="The length of the comment in words", 
        type="integer", 
    ),
    AttributeInfo(
        name="parent_id",
    ...
    AttributeInfo(
        name="is_op",
        description="Whether the comment is made by the original poster",
        type="string"
    ),
     AttributeInfo(
        name="score",
        description="A numerical score for the comment",
        type="integer"
    ),
     AttributeInfo(
        name="forum",
        description="The name of the community (subreddit) the comment was made in",
        type="string"
    ),
     AttributeInfo(
        name="title",
        description="The title of the post the comment was made in",
        type="string"
    ),
     AttributeInfo(
        name="date",
  ...
  retriever = SelfQueryRetriever.from_llm(llm, langchain_chroma, document_content_description, metadata_field_info,verbose=True)
  ...
  output = retriever.get_relevant_documents(prompt, n_results=100)
  print(output)

  # )
  # qa = RetrievalQA.from_chain_type(llm=OpenAI(), chain_type="stuff", vectorstore=langchain_chroma)
  # print(qa.run(prompt))
  # print(stop)
  # docs = langchain_chroma.similarity_search(query=query, n_results=10,query_embeddings="text-embedding-ada-002")
  # docs = langchain_chroma.get(where={"comment": "I think that gmos are bad"})
